...
paymentMethodForm.addEventListener('submit', (e) => {
  if (
    !card1.classList.contains('selected') &&
    !card2.classList.contains('selected')
  ) {
    setErrorFor(card1, 'Please select a payment option');
  ...
  if (card1.classList.contains('selected') && checkbox.checked) {
    successfulMessage.classList.add('active');
    e.preventDefault();
  } else if (card2.classList.contains('selected') && checkbox.checked) {
